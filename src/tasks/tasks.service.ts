...
export class TasksService {
  ...
  async toggleComplete(id: string, tenantId: string, userId?: string) {
  ...
  async getTask(id: string, tenantId: string, userId?: string) {
    const task = await this.repo.findOne({
      ...
      relations: ['assignee', 'project', 'labels'],
    });
  ...
  async getStats(projectId: string, tenantId: string, userId?: string) {
    const tasks = await this.getTasks(projectId, tenantId, userId);
    ...
    const todo = tasks.filter((t) => t.status === TaskStatus.PENDING).length;
    ...
    const overdue = tasks.filter((t) => {
      ...
      return new Date(t.dueDate) < today && t.status !== TaskStatus.COMPLETED;
    }).length;
  ...
  async createTask(
    ...
    if (labelIds !== undefined) {
      ...
      if (uniqueLabelIds.length > 0) {
        labels = await this.labelRepo
          ...
          .innerJoin('label.project', 'project')
    ...
    if (milestoneId) {
      ...
      if (!milestone) {
        throw new BadRequestException('Milestone is invalid for this project');
      }
    }
  ...
  async getTasks(
    ...
    const sortColumn =
      allowedSortFields[filters?.sortBy ?? ''] ?? 'task.dueDate';
  ...
  async updateTask(
    ...
    if (oldStatus !== task.status) {
      ...
      if (savedTask.assignee && savedTask.assignee.id !== user.id) {
  ...
  async deleteTask(id: string, tenantId: string, userId?: string) {
    const task = await this.repo.findOne({
      where: {
        id,
        project: {
